<template>
	<div class="indexContainer">
		...
		<scroller style="top: 50px" :on-refresh="refresh" :on-infinite="infinite">
	    ...
	    methods: {
			...
			*/
			...
			onItemClick(item){
				if(item.active){
					item.active = false
					let a = _.sortBy(this.newProJson.prodTag.split("#"),item=>{
						return item
					})
					let b = _.without(a, item.keywordNo)
					this.newProJson.prodTag = b.join("#")

				}else{
					item.active=true
					this.recordJson.prodNo = item.keywordNo
					this.recordJson.prodName = item.advertiseName
					this.recordJson.opType = '42100008'//招财猫标签点击次数：42100008
					this.countApply()
					this.newProJson.prodTag += this.newProJson.prodTag?`#${item.keywordNo}`:item.keywordNo
					let a = _.sortBy(this.newProJson.prodTag.split("#"),item=>{
						return item
					})
					this.newProJson.prodTag = a.join("#")
				}
				this.getNewProList()
			}		
	    }
