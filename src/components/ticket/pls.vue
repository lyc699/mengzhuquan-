    ...
    <div class="detail">
        ...
        <div v-if="isShowFirst" class="ticket">
            <group class="head-cell">
                <cell>
                    <p slot="title" class="ticketName">排列三<span>第<b>{{baiscInfo.lottery_no}}</b>期</span><span class="ticketDate">{{baiscInfo.lottery_date}}</span></p>
                    <p slot="inline-desc">
        ...
        <div v-else class="ticket allHistory">
            <group class="head-cell" v-for="(item,index) in lotteryResList" :key="index" @click.native="queryDetail(item.lottery_no)">
                <cell is-link>
                    ...
                    <p slot="inline-desc">
                        <span v-for="(item1,index) in item.lottery_res.split(',')" :key="index" class="num">{{item1}}</span>
                    </p>
                </cell>
            </group>
        </div>
...
export default {
    ...
    methods:{
        getDetail(lottery_no){
            this.$axios.post(this.$api.queryLotteryInfo, {params:{"lottery_id": 'pls',lottery_no:lottery_no?lottery_no:this.$route.query.lottery_no}})
    		.then(response => {
        ...
        getList(){
            this.$axios.post(this.$api.queryLotteryHistory, {params:{"lottery_id": 'pls'}})
    		.then(response => {
    			if(response.code === "0000") {
