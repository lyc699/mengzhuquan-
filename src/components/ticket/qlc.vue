    ...
    <div class="detail">
        ...
        <div v-if="isShowFirst" class="ticket">
            <group class="head-cell">
                <cell>
                    <p slot="title" class="ticketName">七乐彩<span>第<b>{{baiscInfo.lottery_no}}</b>期</span><span class="ticketDate">{{baiscInfo.lottery_date}}</span></p>
                    <p slot="inline-desc">
...
export default {
    ...
    methods:{
        getDetail(lottery_no){
            this.$axios.post(this.$api.queryLotteryInfo, {params:{"lottery_id": 'qlc',lottery_no:lottery_no?lottery_no:this.$route.query.lottery_no}})
    		.then(response => {
        ...
        getList(){
            this.$axios.post(this.$api.queryLotteryHistory, {params:{"lottery_id": 'qlc'}})
    		.then(response => {
    			if(response.code === "0000") {
