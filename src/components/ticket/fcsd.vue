    ...
    <div class="detail">
        ...
        <div v-if="isShowFirst" class="ticket">
            <group class="head-cell">
                <cell>
                    <p slot="title" class="ticketName">福彩3D<span>第<b>{{baiscInfo.lottery_no}}</b>期</span><span class="ticketDate">{{baiscInfo.lottery_date}}</span></p>
                    <p slot="inline-desc">
                        <span class="num" v-for="(item, index) in baiscInfo.lottery_res" :key="index">{{item}}</span>
                    </p>
                </cell>
                <div class="flex">
                    <div style="width:100%">
        ...
        <div v-else class="ticket allHistory">
            <table>
                ...
                <tbody>
                    <tr v-for="(item,index) in lotteryResList" :key="index" @click="queryDetail(item.lottery_no)">
                        <td>{{item.lottery_no}}</td>
                        <td>{{item.lottery_res}}</td>
                        <td class="hasAfter"></td>
                    </tr>
                </tbody>
            </table>
        </div>
...
export default {
    ...
    methods:{
        getDetail(lottery_no){
            this.$axios.post(this.$api.queryLotteryInfo, {params:{"lottery_id": 'fcsd',lottery_no:lottery_no?lottery_no:this.$route.query.lottery_no}})
    		.then(response => {
        ...
        getList(){
            this.$axios.post(this.$api.queryLotteryHistory, {params:{"lottery_id": 'fcsd'}})
    		.then(response => {
    			if(response.code === "0000") {
